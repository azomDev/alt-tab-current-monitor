...
export default class AltTabCurrentMonitorPreferences extends ExtensionPreferences {
    async fillPreferencesWindow(window: Adw.PreferencesWindow): Promise<void> {
        ...
        const useMouseMonitor = new Adw.SwitchRow({
            ...
            subtitle: _(
                'When enabled, the "current monitor" is the one where your mouse pointer is located. When disabled, it\'s the monitor with the focused window.',
            ),
        });
        ...
        const behaviorGroup = new Adw.PreferencesGroup({
            ...
            description: _(
                "Configure how windows behave across workspaces and monitors. If focus is jumping unexpectedly between monitors, try adjusting these settings.",
            ),
        });
        ...
        const enableDebugging = new Adw.SwitchRow({
            ...
            subtitle: _(
                "When enabled, detailed debugging information will be logged to the journal. Use `journalctl -f -o cat /usr/bin/gnome-shell` to view logs.",
            ),
        });
